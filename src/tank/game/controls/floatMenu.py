...
from tank.game.defines import MENU_BACKGROUND, MENU_ENV_BACKGROUND
...
class FloatMenu(Control):
    ...
    def __init__(
        self, targetUI: Surface, width: float, height: float, content: Control
    ):
    ...
    def update(self, delta: float):
        ...
        if self.__isMenuShow:
            ...
            self.__envMaskAlpha = int(
                easeLinear(8 * delta, self.__envMaskAlpha, 255, 1)
            )
        else:
            self.__menuBottom = easeLinear(
                8 * delta,
            ...
            self.__envMaskAlpha = int(
                easeLinear(8 * delta, self.__envMaskAlpha, -0.3, 1)
            )
