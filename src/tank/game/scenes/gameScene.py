...
from pygame import K_ESCAPE, KEYDOWN, QUIT, Rect, Surface
from pygame.event import Event
from pymunk.pygame_util import DrawOptions

# from game.gameLoop import GameLoop
from tank.game.ai import AI
...
from tank.game.controls.textbox import TextBox
...
from tank.game.events.timerManager import TimerManager
from tank.game.gameItemManager import GameItemManager
...
from tank.game.gameObject import GameObject, GameObjectData, GameObjectFactory
from tank.game.keyPressedManager import KEY_PRESS_TYPE
from tank.game.operateable import Operation
from tank.game.scenes.scene import Scene
from tank.game.spaces.gameObjectSpace import GameObjectSpace
from tank.game.tank import TANK_COLOR, Tank, TankData
from tank.game.weapons.weaponFactory import WEAPON_TYPE
from tank.online.onlineData import GameUpdateData
...
class GameScene(Scene):
    ...
    def __init__(self, config: GameSceneConfig):
        ...
        self.__scores = [
            0 for i in range(self.__config.aiNum + self.__config.playerNum)
        ]
    ...
    def __initGameMenu(self):
        ...
        def __backToStart():
            from ..sceneManager import SCENE_TYPE, SceneManager
    ...
    def __gameMapAdded(self):
        if self.__gameItemManager is not None:
            ...
            self.__gameItemManager = GameItemManager(
                self.__gameMap, self.__timerManager
            )
        if self.gameObjectSpace is not None:
            self.gameObjectSpace.spaceRegion = Rect(
                0,
                0,
                self.__gameMap.surface.get_width(),
                self.__gameMap.surface.get_height(),
            )
        # 随后加载坦克
        self.__initTanks()
    ...
    def updateScoreBoard(self, delta: float):
        ...
        basePoint = (
            (self.__scoreUI.get_width() - SCORE_PART_WIDTH * len(self.__tanks)) / 2,
            36,
        )
        for i, tank in enumerate(self.__tanks):
            self.__scoreUI.blit(
                tank.surface, (basePoint[0] + i * SCORE_PART_WIDTH, basePoint[1])
            )
            self.__scoreUI.blit(
