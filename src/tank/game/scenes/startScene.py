...
from tank.game.scenes.gameScene import GameSceneConfig
...
class StartScene(Scene):
    ...
    def __init__(self):
        ...
        img = image.load("src/tank/assets/background.png").convert_alpha()
                ...
                [
                    ...
                    Selection(
                        lambda: "幽灵子弹速度增长率 {0:.2f}".format(
                            GlobalSettingsManager.getGameSettings().ghostSpeedIncreaseRate
                            * 100
                        ),
                        SELECTION_HEIGHT,
                        lambda: None,
    ...
    def update(self, delta: float):
        ...
        self.__ui.blit(
            self.__background,
            self.__background.get_rect(center=self.__ui.get_rect().center),
        )

        self.__ui.blit(
            self.__title, ((self.ui.get_width() - self.__title.get_width()) / 2, 48)
        )
