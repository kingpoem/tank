...
from tank.game.defines import BACKGROUND, WALL_FILTER
...
class GameMapData(GameObjectData):
    ...
    def __init__(self, width: int, height: int):
        ...
        self.__mapData = [
            [MAP_PLOT_TYPE.MAP_EMPTY for x in range(self.__width)]
...
class GameMap(GameObject):
    ...
    def __reGenMapObject(self):
        ...
        self.surface = pygame.Surface(
            (
                MARGIN_X * 2 + PLOT_WIDTH * self.__map.width,
                MARGIN_Y * 2 + PLOT_HEIGHT * self.__map.height,
            )
        )
        self.surface.fill(BACKGROUND)

        def __getUpRectPoints(
        ...
        for x in range(self.width):
            for y in range(self.height):
                if self.__map[x, y] == MAP_PLOT_TYPE.MAP_BLOCK:
                    ...
                    if (
                        y != self.height - 1
                        and self.__map[x, y + 1] == MAP_PLOT_TYPE.MAP_BLOCK
                    ):
                        gfxdraw.filled_polygon(
                            self.surface,
                    ...
                    if (
                        x != self.width - 1
                        and self.__map[x + 1, y] == MAP_PLOT_TYPE.MAP_BLOCK
                    ):
                        gfxdraw.filled_polygon(
                            self.surface,
