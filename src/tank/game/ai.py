...
from tank.game.gameMap import GameMap, MAP_PLOT_TYPE
...
class AI:
    ...
    def __worldToGrid(self, worldPos) -> Tuple[int, int]:
        ...
        将世界坐标转换为网格坐标
        """
        from tank.game.gameMap import MARGIN_X, MARGIN_Y, PLOT_WIDTH, PLOT_HEIGHT
    ...
    def __astar(self, start: Tuple[int, int], goal: Tuple[int, int]) -> List[Tuple[int, int]]:
        """
        A*寻路算法实现
        """
        ...
        def getNeighbors(pos: Tuple[int, int]) -> List[Tuple[int, int]]:
            ...
            for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
                nx, ny = x + dx, y + dy
                if (0 <= nx < self.__gameMap.width and 
                    0 <= ny < self.__gameMap.height and
                    self.__gameMap.map[nx, ny] == MAP_PLOT_TYPE.MAP_EMPTY):
    ...
    def __performShooting(self, delta: float):
        ...
        if (currentTime >= self.__shootCooldown and 
            self.__currentBullets < self.__maxBullets and 
            self.__targetTank):
